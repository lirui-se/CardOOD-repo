...
/* Hook for call back after ExplainPrintPlan. */
ExplainPrintPlan_CallBack_hook_type ExplainPrintPlan_CallBack_hook = NULL;
				...
				QueryEnvironment *queryEnv)
{
	...
	else
	{
		...
		/* plan the query */
        /* Add query string to query->qstr. */
        query->qstr = (char *)palloc0((strlen(queryString) + 2) * sizeof(char));
        strcpy(query->qstr, queryString);
			   ...
			   QueryEnvironment *queryEnv, const instr_time *planduration)
{
    ...
    if(ExplainPrintPlan_CallBack_hook) {
        (*ExplainPrintPlan_CallBack_hook)(es, queryDesc);
    }
