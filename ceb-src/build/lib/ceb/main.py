...
def get_featurizer(args, cfg, trainqs, valqs, testqs, all_evalqs):
    ...
    end_time = time.time()
    print(f"setup elapsed time = {end_time - start_time} s.")
    ...
    end_time = time.time()
    # 0.052 s
    print(f"Update_max_sets elapsed time = {end_time - start_time} s.")

    featurizer.update_workload_stats(trainqs+valqs+testqs+all_evalqs)
    end_time = time.time()
    # 81.99 s
    print(f"Update_workload_stats elapsed time = {end_time - start_time} s.")

    featurizer.init_feature_mapping()
    end_time = time.time()
    # 81.99s
    print(f"init_feature_mapping elapsed time = {end_time - start_time} s.")
    ...
    if cfg["featurizer"]["feat_onlyseen_maxy"]:
        ...
        featurizer.update_ystats(trainqs+valqs+testqs+all_evalqs,
                max_num_tables = cfg["model"]["max_num_tables"])
    end_time = time.time()
    print(f"update_ystats elapsed time = {end_time - start_time} s.")

    featurizer.update_seen_preds(trainqs)
    end_time = time.time()
    # 83.465s
    print(f"update_seen_preds elapsed time = {end_time - start_time} s.")
    return featurizer
...
def main():
    ...
    if cfg["eval"]["use_wandb"]:
        ...
        # Rui: when debugging, use offline mode to disable to intereactive part. 
        wandb.init("ceb", config=wandbcfg,
                tags=wandb_tags, mode='offline')
    ...
    trainqs = load_qdata_parallel(train_qfns)
    # trainqs = load_qdata(train_qfns)
    sec= 0
    for t in trainqs:
        ssg = t["subset_graph"]
        for node in ssg.nodes():
            att = ssg.nodes()[node]
            sec += att["exec_time"]["actual"]
    print(f"{sec} seconds, {sec/3600} hours, {sec/3600/24} days.")
    valqs = load_qdata_parallel(val_qfns)
    # valqs = load_qdata(val_qfns)
    testqs = load_qdata_parallel(test_qfns)
