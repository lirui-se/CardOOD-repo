...
class NN(CardinalityEstimationAlg):
    ...
    def __init__(self, model_params, *args, **kwargs):
        ...
        # self parameters will overwrite dsb config files.
        for k, val in kwargs.items():
            self.__setattr__(k, val)

        # pdb.set_trace()
        # predict = NN.net(X), not NN(X)
        self.is_net_wrapper = True
        ...
        if self.loss_func_name == "qloss":
            ...
            self.load_query_together = False
            print("algs.py::class(NN)::__init__:", "lr = ", self.lr, "mb_size = ", self.mb_size)
    ...
    def init_net(self, sample):
        # self._init_net is implemented by the child class of NN
    ...
    def get_trainds(self, training_samples, **kwargs):
        if "subplan_mask" in kwargs:
            subplan_mask = kwargs["subplan_mask"]
        else:
            subplan_mask = None

        trainds = self.init_dataset(training_samples,
                self.load_query_together,
                max_num_tables = self.max_num_tables,
                load_padded_mscn_feats=self.load_padded_mscn_feats,
                feat_dir = self.featurizer.feat_dir,
                subplan_mask = subplan_mask,
                with_info = False
                )
        return trainds

    def get_grp_trainds(self, training_samples, **kwargs):
        if "subplan_mask" in kwargs:
            subplan_mask = kwargs["subplan_mask"]
        else:
            subplan_mask = None

        trainds = self.init_grp_dataset(training_samples,
                    self.load_query_together,
                    max_num_tables = self.max_num_tables,
                    load_padded_mscn_feats=self.load_padded_mscn_feats,
                    feat_dir = self.featurizer.feat_dir,
    ...
    def get_train_env(self, training_samples, part_key: str = 'num_templates'):
        class CEBJoinQueryEnvironment(object):
            def __init__(self, datasets):
                self.datasets = datasets

        assert part_key == 'num_templates', f"only support num_templates now."
        partition_query_indices = {}
        for i , qrep in enumerate(training_samples):
            tpl = qrep['template_name']
            if tpl not in partition_query_indices:
                partition_query_indices[tpl] = list()
            partition_query_indices[tpl].append(i)

        datasets = list()
        for query_indices in partition_query_indices.values():
            cur_qrep = []
            for i in query_indices:
                cur_qrep.append(training_samples[i])
            datasets.append(self.get_trainds(cur_qrep))

        return CEBJoinQueryEnvironment(datasets)

    def get_ttt_trainds(self, training_samples, num_negs, **kwargs):
        if "subplan_mask" in kwargs:
            subplan_mask = kwargs["subplan_mask"]
        else:
            subplan_mask = None

        trainds = self.init_ttt_dataset(training_samples,
                self.load_query_together,
                max_num_tables = self.max_num_tables,
                load_padded_mscn_feats=self.load_padded_mscn_feats,
                feat_dir = self.featurizer.feat_dir,
    ...
    def get_trainloader(self, trainds):
        trainloader = data.DataLoader(trainds,
    ...
    def get_testds(self, testing_samples, sort_sub=True, **kwargs):
        testds = self.init_dataset(testing_samples,
                False,
                load_padded_mscn_feats=self.load_padded_mscn_feats,
                feat_dir = self.featurizer.feat_dir,
                subplan_mask = None,
    ...
    def get_ttt_testds(self, testing_samples, num_negs, sort_sub=True, **kwargs):
        testds = self.init_ttt_dataset(testing_samples,
                False,
                load_padded_mscn_feats=self.load_padded_mscn_feats,
                feat_dir = self.featurizer.feat_dir,
                subplan_mask = None,
    ...
    def get_testloader(self, testds):
    ...
    def train_one_epoch(self):
        ...
        for idx, (xbatch, ybatch, info) in enumerate(self.trainloader):
            ...
            # self.random_bitmap_idx = 0
            if self.random_bitmap_idx:
                idxs = torch.randperm(xbatch["join"].shape[-1])
                xbatch["join"] = xbatch["join"][:,:,idxs]

            # self.onehot_dropout = 0
    ...
    def test(self, test_samples, **kwargs):
        ...
        start = time.time()
        # preds are predict of the model. 
