...
class Featurizer():
    def __init__(self, **kwargs):
        ...
        self.eqpat = re.compile(r'\s*([a-zA-z]+[a-zA-z_0-9\.]*)\s*=\s*(.+)\s*')
        self.nepat = re.compile(r'\s*([a-zA-z]+[a-zA-z_0-9\.]*)\s*!=\s*(.+)\s*')
        self.ltpat = re.compile(r'\s*([a-zA-z]+[a-zA-z_0-9\.]*)\s*<=\s*(.+)\s*')
        self.gtpat = re.compile(r'\s*([a-zA-z]+[a-zA-z_0-9\.]*)\s*>=\s*(.+)\s*')
        self.btpat = re.compile(r'\s*([a-zA-z]+[a-zA-z_0-9\.]*)\s*(between|BETWEEN)\s*(.+)\s*(AND|and)\s*(.+)\s*')
        self.inpat = re.compile(r'\s*([a-zA-z]+[a-zA-z_0-9\.]*)\s*(in|IN)\s*\((.+)\)\s*')
        self.likepat = re.compile(r'\s*([a-zA-z]+[a-zA-z_0-9\.]*)\s*(like|LIKE)\s*(.+)\s*')
    ...
    def update_column_stats(self, qreps):
        print(f"updating column statistics...")
        for qi, qrep in enumerate(qreps):
            if qi % 100 == 0:
                print(f"update column statistics: qi = {qi}")
                ...
                #             try:
                #                 col, op, val = self.parse_predicates(p)
    ...
    def update_seen_preds(self, qreps):
        ...
        for qrep in qreps:
            ...
            if self.sample_bitmap:
                print("self.sample_bitmap = ", self.sample_bitmap)
    ...
    def parse_predicates(self, p):
        meq = self.eqpat.match(p)
        mne = self.nepat.match(p)
        mlt = self.ltpat.match(p)
        mgt = self.gtpat.match(p)
        mbt = self.btpat.match(p)
        min = self.inpat.match(p)
        mlike = self.likepat.match(p)
    ...
    def update_workload_stats(self, qreps):
        for qrep in qreps:
            ...
            for join_str in joins:
                ...
                join_str = self.join_str_to_real_join(join_str)
                if not self.feat_separate_alias and "dsb" not in self.db_name:
    ...
    def _init_pred_featurizer_set(self):
        ...
        for cidx, col_name in enumerate(all_cols):
            ...
            col_alias, col = self._get_table_and_col(col_name)
            ...
            real_col = self.aliases[col_alias] + "." + col
    ...
    def setup(self,
        ...
        # change the
        if not self.feat_separate_alias:
            if "synth" not in self.db_name and "dsb" not in self.db_name:
                newstats = {}
    ...
    def _handle_join_features(self, join_str):
        ...
        if use_onehot:
            ...
            if keys not in self.join_featurizer:
                assert False, "join_str: {} not found in featurizer".format(join_str)
                # print(join_str)
    ...
    def _handle_single_col(self, col, val,
        ...
        if self.heuristic_features:
            ...
            # self.feat_separate_like_ests = 0
            if self.feat_separate_like_ests:
                assert pfeats[-3] == 0.0
                assert pfeats[-4] == 0.0
                if "like" in cmp_op:
    ...
    def get_subplan_features_set(self, qrep, subplan, bitmaps=None,
        ...
        true_card = subsetgraph.nodes()[subplan]['cardinality']['actual']
        try:
            if true_card >= 10 ** 8:
                return None
            if true_card <= 1:
        ...
        if DEBUG:
            print(" ========= inside get_subplan_features_set ========= ")
            print(" tables: ", subplan)
        alltablefeats = []
                ...
                for alias1 in subplan:
                    for alias2 in subplan:
                        ...
                        if ekey in joingraph.edges():
                            join_str_list = joingraph.edges()[ekey]["join_condition"].split(",")
        ...
        for alias in subplan:
            ...
            # alias must be as base table.
            node_key = tuple([alias])

            # self.true_base_cards = 0
            if self.true_base_cards:
                alias_est = self._get_true_est(subsetgraph.nodes()[node_key])
                subp_est = self._get_true_est(subsetgraph.nodes()[subplan])
            else:
            ...
            for ci, col in enumerate(aliasinfo["pred_cols"]):
                ...
                # updated
                if not self.feat_separate_alias \
                        and "synth" not in self.db_name and "dsb" not in self.db_name:
                    col = ''.join([ck for ck in col if not ck.isdigit()])

                if col not in self.column_stats:
                    print(f"col not in self.column_stats! {col}, {self.column_stats.keys()}")
                    continue
    ...
    def get_ttt_subplan_features_set(self, qrep, subplan, bitmaps=None,
            join_bitmaps=None,
            subset_edge=None,
            num_negs=3):
        '''
        @ret: {}
            ...
            subplan we're trying to featurize.
        '''
        # prepare neg qrep datasets. 
        # line 3154: def get_ttt_subplan_features()
        assert isinstance(subplan, tuple)
        featdict = {}
        neg_featdict = {}
        subsetgraph = qrep["subset_graph"]
        ...
        if 'cardinality' in subsetgraph.nodes()[subplan].keys():
            true_card = subsetgraph.nodes()[subplan]['cardinality']['actual']
            try:
                if true_card >= 10 ** 8:
                    return None, None
        ...
        if DEBUG:
            print(" ========= inside get_ttt_subplan_features_set ========= ")
            print(" tables: ", subplan)
            print(subplan)

        # step 1: table features. 
        alltablefeats = []
        neg_table_feats = []
        if self.table_features:
            ## table features
            ...
            for _ in range(num_negs):
                neg_table_feats.append(alltablefeats)

        # print(alltablefeats)
        # pdb.set_trace()
        featdict["table"] = alltablefeats
        neg_featdict["table"] = neg_table_feats
        ...
        if self.join_features:
            ...
            if len(alljoinfeats) == 0:
                alljoinfeats.append(np.zeros(self.join_features_len))
            for _ in range(num_negs):
                neg_join_feats.append(alljoinfeats)

        featdict["join"] = alljoinfeats
        neg_featdict["join"] = neg_join_feats
        ...
        for alias in subplan:
            ...
            for ci, col in enumerate(aliasinfo["pred_cols"]):
                # we should have updated self.column_stats etc. to be appropriately
                # updated
                # self.feat_separate_alias = 0
                if not self.feat_separate_alias and "synth" not in self.db_name \
                        and "dsb" not in self.db_name:
                    col = ''.join([ck for ck in col if not ck.isdigit()])
                

                if col not in self.column_stats:
                    print(f"col not in column_stas! {col}, {self.column_stats.keys()}")
                    pdb.set_trace()
                    continue
                ...
                val_pointer = aliasinfo["pred_vals"][ci]
                # replace the none value with min / max value. 
                if continuous:
                    if cmp_op == 'gt':
                        assert val_pointer[0] is not None, f"{val_pointer}, {qrep['sql']}" 
                        if val_pointer[1] is not None:
                            assert val_pointer[0] <= val_pointer[1]
                        else:
                            val_pointer[1] = self.column_stats[col]['max_value']
                    elif cmp_op == 'lt':
                        assert val_pointer[1] is not None,f"{val_pointer}, {qrep['sql']}"
                        if val_pointer[0] is not None:
                            assert val_pointer[0] <= val_pointer[1]
                        else:
                            val_pointer[0] = self.column_stats[col]['min_value']
                ...
                pred = (col,allvals, alias_est, subp_est, cmp_op, continuous, jobquery)
                if DEBUG:
                    print(pred)
                pfeats = self._handle_single_col(*pred)
                allpredfeats += pfeats
                all_preds[alias].append(pred)
            
        # prepare neg_feats. 
        def sub_query_sample(pred_tuples):
            l = len(pred_tuples)
            if l < 1:
                return pred_tuples
            num_sample_pred = random.randint(1, l)
            sample_pred_idx = random.sample(list(range(l)), k=num_sample_pred)
            new_pred_list = list()
            for idx, pred in enumerate(pred_tuples):
                if idx not in sample_pred_idx:
                    new_pred_list.append(pred)
                    continue
                col, allvals, alias_est, subp_est, cmp_op, continuous, jobquery = pred
                if not continuous or not isinstance(allvals, list):
                    new_pred_list.append(pred)
                    continue
                lower, upper = allvals[0], allvals[1]
                try:
                    assert lower <= upper, f"lower > upper, col = {col}..."
                ...
                if upper == mid == lower:
                    ...
                    new_pred_list.append( (col, [lower, mid], alias_est, subp_est, cmp_op, continuous, jobquery ) )
                else:
                    new_pred_list.append( (col, [mid, upper], alias_est, subp_est, cmp_op, continuous, jobquery ) )
        ...
        for _ in range(num_negs):
            temp_preds = []
            temp_pred_feats = []
            for ci in all_preds.keys():
                preds = sub_query_sample(all_preds[ci])
                temp_preds += preds
            for pred in temp_preds:
                col, allvals, alias_est, subp_est, cmp_op, continuous, jobquery = pred
                pfeats = self._handle_single_col(col, allvals, alias_est, subp_est, cmp_op, continuous, jobquery)
                temp_pred_feats += pfeats
            neg_predfeats.append(temp_pred_feats)
        ...
        if len(allpredfeats) == 0:
            allpredfeats.append(np.zeros(self.max_pred_len))
        for e in neg_predfeats:
            if len(e) == 0:
                e.append(np.zeros(self.max_pred_len))
        ...
        if not len(allpredfeats) <= self.max_pred_vals:
            ...
            pdb.set_trace()
        for e in neg_predfeats:
            if not len(e) <= self.max_pred_vals:
                print(len(e), self.max_pred_vals)
                pdb.set_trace()

        if self.pred_features:
            featdict["pred"] = allpredfeats
            neg_featdict["pred"] = neg_predfeats
        else:
            featdict["pred"] = []
        ...
        if self.global_features:
            global_features = self.get_global_features(subplan,
                    ...
                    qrep["join_graph"], subset_edge=subset_edge)
            for _ in range(num_negs):
                neg_global_features.append(global_features)

        featdict["flow"] = global_features
        neg_featdict["flow"] = neg_global_features

        # import pdb; pdb.set_trace()
        return featdict, neg_featdict

    def dump_featdict(self, featdict, depth=0):
        if isinstance(featdict, dict):
            print("dict{", end="")
            for k in featdict.keys():
                print(k + ":", end="")
                self.dump_featdict(featdict[k], depth+1)
            print("}", end="")
        elif isinstance(featdict, list):
            print(f"list {len(featdict)} (", end="")
            for e in featdict[0:5]:
                self.dump_featdict(e, depth+1)
            print(")",end="")
        elif isinstance(featdict, tuple):
            print(f"tuple {len(featdict)} (", end="")
            for e in featdict[0:5]:
                self.dump_featdict(e, depth+1)
            print(")",end="")
        elif isinstance(featdict, torch.Tensor):
            print(f"{featdict.shape}",end="")
        elif isinstance(featdict, np.ndarray):
            print(f"{featdict.shape}",end="")
        else:
            print(f"{type(featdict)}",end="")
        if depth == 0:
            print("")

    def get_ttt_subplan_features_combined(self, qrep, subplan, bitmaps=None,
            join_bitmaps=None, num_negs=3):
        assert isinstance(subplan, tuple)
        featvectors = []
    ...
    def get_ttt_subplan_features(self, qrep, node, bitmaps=None,
            join_bitmaps=None, num_negs=3):
        '''
        @subsetg:
        ...
        if self.featurization_type == "combined":
            x, x_neg = self.get_ttt_subplan_features_combined(qrep, node, bitmaps=bitmaps, join_bitmaps=join_bitmaps, num_negs=num_negs)
        elif self.featurization_type == "set":
            x, x_neg = self.get_ttt_subplan_features_set(qrep, node, bitmaps=bitmaps, join_bitmaps=join_bitmaps, num_negs=num_negs)
            if x is None:
                self.timeout_query += 1
                self.total_query += 1
    ...
    def unnormalize_with_log(self, y, total):
        if self.ynormalization == "logwhitening":
            est_card = np.exp((y*self.stdy) + self.meany)
        elif self.ynormalization == "whitening":
            est_card = (y*self.stdy) + self.meany
            ...
            ## wrong?
            est_card = (y*(self.max_val-self.min_val) + self.min_val)
    ...
    def normalize_val(self, val, total):
        ...
        if self.ynormalization == "logwhitening":
            ...
            # picked! 
            # print("=============")
            # print(float(val), self.min_val, self.max_val, (np.log(float(val)) - self.min_val) / (self.max_val-self.min_val))
    ...
    def _get_table_and_col(self, column):
        if "." in column:
            curalias = column[0:column.find(".")]
            curalias = curalias.replace(" ", "")
            curcol = column[column.find(".") + 1:]
            curcol = curcol.replace(" ", "")
        else:
            curalias = column[0:column.find("_")]
            curalias = dsb_col_map[curalias]
            curcol = column
            curcol = curcol.replace(" ", "")
        return curalias, curcol

    def _get_col_names(self, column):
        curalias, curcol = self._get_table_and_col(column)
        curcol_wrapper = '"' + curcol + '"'
        curtab = self.aliases[curalias]
        uniq_col_id = curtab + "." + curcol_wrapper
        query_col_id = curalias + "." + curcol_wrapper
        return curalias, curcol, curcol_wrapper, curtab, uniq_col_id, query_col_id
    ...
    def _update_stats(self, qrep):
        ...
        for join in joins:
            ...
            for jkey in keys:
                ...
                curalias, curcol = self._get_table_and_col(jkey)
        ...
        for column in cur_columns:
            ...
            # e.g.: column = d1.d_year
            # 
            # curalias = d1
            # curcol = d_year
            # curcol_wrapper = "d_year"
            # curtab = date_dim
            # uniq_col_id = date_dim."d_year"
            # query_col_id = d1."d_year"
            curalias, curcol, curcol_wrapper, curtab, uniq_col_id, query_col_id = self._get_col_names(column)
            ...
            for si,tmp in enumerate(self.join_key_stat_tmps):
                sname = self.join_key_stat_names[si]
                execcmd = tmp.format(TABLE=curtab, COL=curcol)
                try:
                    val = float(self.execute(execcmd)[0][0])
                except:
                    val = 0.0
            ...
            print(f"Collecting col info for {column}")
            column_stats = {}
            if curalias in self.aliases:
                table = ALIAS_FORMAT.format(TABLE = self.aliases[curalias],
                                    ALIAS = curalias)
            else:
                print(curalias)
                pdb.set_trace()
