...
from asset.groupdro import GroupQueryDataset, GroupJoinQueryDataset
from asset.dann import DANNTrainer, DANNModel
...
def init_args(arg_list: list):
    ...
    parser.add_argument('--lambda_dann', type=float, default=0.001, # 1e-3 for MSCN
...
def prepare_table_datasets(args):
    ...
    if args.model_type == 'MSCN':
        train_dataset = GroupQueryDataset(table=table, queries=train_queries, cards=train_cards, query_infos= train_query_infos, encoder_type='onehot')
        test_dataset = QueryDataset(table=table, queries=test_queries, cards=test_cards, encoder_type='onehot')

    else:
        train_dataset = GroupQueryDataset(table=table, queries=train_queries, cards=train_cards, query_infos= train_query_infos, encoder_type='dnn')
...
def prepare_schema_datasets(args):
    ...
    train_queries, train_cards, train_query_infos, test_queries, test_cards, test_query_infos = (
        uneven_train_test_split(all_queries=all_queries, all_cards=all_cards, all_query_infos=all_query_infos,
                                skew_split_keys='num_table', train_frac = args.train_frac, skew_ratio=args.skew_ratio))
    if args.model_type == 'MSCN':
        train_dataset = GroupJoinQueryDataset(schema=schema, queries=train_queries, cards=train_cards,
                                              query_infos=train_query_infos, encoder_type='onehot')
        test_dataset = JoinQueryDataset(schema=schema, queries=test_queries, cards=test_cards, encoder_type='onehot')

    else:
        train_dataset = GroupJoinQueryDataset(schema=schema, queries=train_queries, cards=train_cards, query_infos= train_query_infos, encoder_type='dnn')
    ...
    if args.is_join_query:
        if args.model_type == 'CEB_MSCN':
            ...
            if args.support_skew:
                train_dataset, train_query_infos, test_dataset, test_query_infos = prepare_schema_datasets(args)
            else:
    ...
    if args.model_type == 'MSCN':
        if args.is_join_query:
            ...
            model = MSCN(train_dataset.feat_dim, args.pred_num_hid, args.pred_num_out, args.mlp_num_hid,
                         return_rep=True)
        dann_model = DANNModel(feat_dim=args.mlp_num_hid, n_groups=train_dataset.n_groups)
    elif args.model_type == 'CEB_MSCN':
        # pdb.set_trace()
        dann_model = DANNModel(feat_dim=130, n_groups=train_dataset.n_groups)
    elif args.model_type == 'DNN':
        model = MLP(in_ch=train_dataset.feat_dim, hid_ch=args.num_hid, out_ch=1, return_rep=True)
        dann_model = DANNModel(feat_dim=args.num_hid, n_groups=train_dataset.n_groups)
    else:
        raise NotImplementedError('Unsupported model type {}'.format(args.model_type))

    if args.model_type == 'CEB_MSCN':
        return model, dann_model, train_dataset, train_query_infos, test_dataset, test_query_infos, debug_dataset, debug_query_infos
    else:
        return model, dann_model, train_dataset, train_query_infos, test_dataset, test_query_infos

class SERVER_DANNModel:
    def __init__(self, arg_list: str):
        self.args = init_args(arg_list)
        args = self.args
        if args.model_type == 'CEB_MSCN':
            self.model, self.dann_model, self.train_dataset, self.train_query_infos,  self.test_dataset, self.test_query_infos, self.debug_dataset, self.debug_query_infos = prepare_model_and_dataset(self.args)
            self.dann_trainer = DANNTrainer(args, self.model.net, self.dann_model, optimizer=self.model.optimizer, model_wrapper=self.model)
            self.base = np.e
        else:
            self.schema = load_schema(self.args.schema_name, self.args.data_path)
            self.query_parser = JoinQueryParser(schema=self.schema)
            self.model, self.dann_model, self.train_dataset, self.train_query_infos, self.test_dataset, self.test_query_infos = prepare_model_and_dataset(self.args, self.schema)
            self.pred_stat = PredictionStatistics()
            self.dann_trainer = DANNTrainer(self.args, self.model, self.dann_model)
            self.base = 2.0
        self.server_info = {}
    ...
    def load_model(self):
        ...
        if not args.support_skew:
            para = '_'.join( [ 'dann', str(args.skew_split_keys), str(args.model_type), str(args.num_negs), str(args.epochs), str('{:.0e}'.format(args.learning_rate)), str(args.batch_size) ] )
        else:
            para = '_'.join( [ 'dann', str(args.skew_ratio), str(args.skew_split_keys), str(args.model_type), str(args.num_negs), str(args.epochs), str('{:.0e}'.format(args.learning_rate)), str(args.batch_size) ] )

        assert args.model_save_path is not None
        assert os.path.exists(args.model_save_path)
        # name = "/home/lirui/codes/PG_CardOOD/CardOOD/ood/run/dann_models/" + para + ".model"
        name = args.model_save_path + "/" + para + ".model"
        print("Model save path:", name)
        if os.path.exists(name):
            self.dann_trainer.model.load_state_dict(torch.load(name))
        else:
            with torch.autograd.detect_anomaly():
                self.dann_trainer.train(self.train_dataset)
            self.save_model(name)
        errors = self.dann_trainer.test(self.test_dataset)
    ...
    def save_model(self, name):
        torch.save( self.dann_trainer.model.state_dict(), name)
    ...
    def predict(self, query_lines: list):
        ...
        test_loader = self.dann_trainer.prepare_test_loader(test_dataset)
        # test_loader = self.dann_trainer.prepare_test_loader(test_dataset, padding_func_type="TypeB")
        # test_loader = self.dann_trainer.prepare_test_loader(test_dataset, padding_func_type="TypeB")
        outputs, labels = list(), list()
        self.dann_trainer.model.eval()
        with torch.no_grad():
            for(x, y) in test_loader:
                x, y = recursive_to_device((x, y), self.args.device)
                rep, _ = self.dann_trainer.model(x)
                output = self.dann_trainer.dann_model.regressor(rep)
...
def main(args):
    pred_stat = PredictionStatistics()
    if args.model_type == 'CEB_MSCN':
        model, dann_model, train_dataset, train_query_infos,  test_dataset, test_query_infos, debug_dataset, debug_query_infos = prepare_model_and_dataset(args)
        dann_trainer = DANNTrainer(args, model.net, dann_model, optimizer=model.optimizer, model_wrapper=model)
        base = np.e
    else:
        model, dann_model, train_dataset, train_query_infos,  test_dataset, test_query_infos  = prepare_model_and_dataset(args)
        dann_trainer = DANNTrainer(args, model, dann_model)
        base = 2.0
    ...
    if args.model_save_path is not None and os.path.exists(args.model_save_path): 
        ...
        if os.path.exists(name):
            dann_trainer.model.load_state_dict(torch.load(name))
        else:
            with torch.autograd.detect_anomaly():
                dann_trainer.train(train_dataset)
            torch.save(dann_trainer.model.state_dict(), name)
    else:
        with torch.autograd.detect_anomaly():
            dann_trainer.train(train_dataset)

    if args.model_type == "CEB_MSCN" and args.dump_prediction == 1:
        errors, test_out = dann_trainer.test(test_dataset, return_out=True)
        train_errors, train_out = dann_trainer.test_train_dataset(train_dataset, return_out=True)
        ...
        # debug_errors = dann_trainer.test(debug_dataset)
    else:
        errors = dann_trainer.test(test_dataset)
    # if args.model_type == 'CEB_MSCN':
    #     debug_errors = dann_trainer.test(debug_dataset, dump=True)
    # train_errors = dann_trainer.test_train_dataset(train_dataset)
    if args.is_join_query:
