...
from asset.groupdro import GroupQueryDataset, GroupJoinQueryDataset, GroupDROTrainer
...
def init_args(arg_list: list):
    ...
    parser.add_argument("--schema_query_path", type=str, default='/home/kfzhao/PycharmProjects/NNGP/queryset/join_title_cast_info_movie_info_movie_companies_movie_info_idx_movie_keyword_10_data_centric_824_FP')
...
def prepare_table_datasets(args):
    ...
    if args.model_type == 'MSCN':
        ...
        print("num groups:", train_dataset.n_groups)
    ...
    if args.is_join_query:
        if args.model_type == 'CEB_MSCN':
            ...
            if args.skew_split_keys == 'template_no' or args.skew_split_keys == 'distri':
                train_dataset, train_query_infos, test_dataset, test_query_infos = prepare_query_pattern_datasets(args)
            else:
                train_dataset, train_query_infos, test_dataset, test_query_infos = prepare_schema_datasets(args)
    else:
        train_dataset, test_dataset, test_query_infos = prepare_table_datasets(args)
    if args.model_type == 'MSCN':
        if args.is_join_query:
...
class GROUPDROModel:
    def __init__(self, arg_list: str):
        ...
        if args.model_type == 'CEB_MSCN':
            ...
            self.base = np.e
            self.groupdro_trainer = GroupDROTrainer(args, self.model.net, optimizer=self.model.optimizer, model_wrapper=self.model)
        else:
            ...
            self.groupdro_trainer = GroupDROTrainer(self.args, self.model)
            self.base = 2.0
        self.server_info = {}
    ...
    def load_model(self):
        args = self.args
        para = '_'.join( [ 'groupdro', str(args.skew_split_keys), str(args.model_type), str(args.num_negs), str(args.epochs), str('{:.0e}'.format(args.learning_rate)), str(args.batch_size) ] )
        assert args.model_save_path is not None
        assert os.path.exists(args.model_save_path)
        # name = "/home/lirui/codes/PG_CardOOD/CardOOD/ood/run/groupdro_models/" + para + ".model"
        name = args.model_save_path + "/" + para + ".model"
        print("Model save path:", name)
        if os.path.exists(name):
            self.groupdro_trainer.model.load_state_dict(torch.load(name))
        else:
            self.groupdro_trainer.train(self.train_dataset)
            self.save_model(name)
        errors = self.groupdro_trainer.test(self.test_dataset)
    ...
    def save_model(self, name):
        torch.save( self.groupdro_trainer.model.state_dict(), name)
    ...
    def predict(self, query_lines: list):
        ...
        test_loader = self.groupdro_trainer.prepare_test_loader(test_dataset)
        # test_loader = self.groupdro_trainer.prepare_test_loader(test_dataset, padding_func_type="TypeB")
        # test_loader = self.groupdro_trainer.prepare_test_loader(test_dataset, padding_func_type="TypeB")
        outputs, labels = list(), list()
        self.groupdro_trainer.model.eval()
        with torch.no_grad():
            for(x, y) in test_loader:
                x, y = recursive_to_device((x, y), self.args.device)
                output = self.groupdro_trainer.model(x)
...
def main(args):
    ...
    pred_stat = PredictionStatistics()
    if args.model_type == 'CEB_MSCN':
        groupdro_trainer = GroupDROTrainer(args, model.net, optimizer=model.optimizer, model_wrapper=model)
        base = np.e
    else:
        groupdro_trainer = GroupDROTrainer(args, model)
        base = 2.0
    ...
    if args.model_save_path is not None and os.path.exists(args.model_save_path): 
        ...
        print("Model save path:", name)
        if os.path.exists(name):
            groupdro_trainer.model.load_state_dict(torch.load(name))
        else:
            with torch.autograd.detect_anomaly():
                groupdro_trainer.train(train_dataset)
            torch.save(groupdro_trainer.model.state_dict(), name)
    else:
        with torch.autograd.detect_anomaly():
            groupdro_trainer.train(train_dataset)
    if args.model_type == "CEB_MSCN" and args.dump_prediction == 1:
        errors, test_out = groupdro_trainer.test(test_dataset, return_out=True)
        train_errors, train_out = groupdro_trainer.test_train_dataset(train_dataset, return_out=True)
        ...
        # debug_errors = groupdro_trainer.test(debug_dataset)
    else:
        errors = groupdro_trainer.test(test_dataset)
    # train_errors = groupdro_trainer.test_train_dataset(train_dataset)
    if args.is_join_query:
