...
from encoder.transform import QueryDataset, JoinQueryDataset, MaskedQueryDataset, MaskedJoinQueryDataset
from encoder.transform import *
from model.model import MLP, MSCN, MSCNJoin
from util import train_test_val_split, query_pattern_train_test_split, uneven_train_test_split, uneven_train_test_split_v2, PredictionStatistics, seed_all
...
def init_args(arg_list: list):
    ...
    parser.add_argument('--p', type=float, default=0.1,
                        help='the probability of masking predicates')
    ...
    assert args.is_join_query in [ 'True', 'true', 'False', 'false', 1, 0 ], "is_join_query must b;ae True or true or 1 or False or false or 0"
...
def prepare_table_datasets(args):
    ...
    if args.model_type == 'MSCN':
        ...
        uneven_train_test_split_v2(all_queries=all_queries, all_cards=all_cards, all_query_infos=all_query_infos,
        ...
        train_queries, train_cards, train_query_infos, test_queries, test_cards, test_query_infos = (
            uneven_train_test_split(all_queries=all_queries, all_cards=all_cards, all_query_infos=all_query_infos,
                                       skew_split_keys='num_predicates', skew_ratio=args.skew_ratio))
    if args.model_type == 'MSCN':
        train_dataset = MaskedQueryDataset(table=table, queries=train_queries, cards=train_cards, p=args.p, encoder_type='onehot')
        test_dataset = QueryDataset(table=table, queries=test_queries, cards=test_cards, encoder_type='onehot')

    else:
        train_dataset = MaskedQueryDataset(table=table, queries=train_queries, cards=train_cards, p=args.p, encoder_type='dnn')
...
def prepare_schema_datasets(args):
    ...
    if args.model_type == 'MSCN':
        train_dataset = MaskedJoinQueryDataset(schema=schema, queries=train_queries, cards=train_cards, p=args.p, encoder_type='onehot')
        test_dataset = JoinQueryDataset(schema=schema, queries=test_queries, cards=test_cards, encoder_type='onehot')
    else:
        train_dataset = MaskedJoinQueryDataset(schema=schema, queries=train_queries, cards=train_cards, p=args.p, encoder_type='dnn')
...
class MASKModel:
    def __init__(self, arg_list: str):
        ...
        if args.model_type == 'CEB_MSCN':
            ...
            self.base = np.e
            self.mask_trainer = ERMTrainer(args, self.model.net, optimizer=self.model.optimizer, model_wrapper=self.model)
        else:
            ...
            self.mask_trainer = ERMTrainer(self.args, self.model)
            self.base = 2.0
        self.server_info = {}
    ...
    def load_model(self):
        args = self.args
        para = '_'.join( [ 'mask', str(args.skew_split_keys), str(args.model_type), str(args.num_negs), str(args.epochs), str('{:.0e}'.format(args.learning_rate)), str(args.batch_size) ] )
        assert args.model_save_path is not None
        assert os.path.exists(args.model_save_path)
        # name = "/home/lirui/codes/PG_CardOOD/CardOOD/ood/run/mask_models/" + para + ".model"
        name = args.model_save_path + "/" + para + ".model"
        print("Model save path:", name)
        if os.path.exists(name):
            self.mask_trainer.model.load_state_dict(torch.load(name))
        else:
            self.mask_trainer.train(self.train_dataset)
            self.save_model(name)
        errors = self.mask_trainer.test(self.test_dataset)
    ...
    def save_model(self, name):
        torch.save( self.mask_trainer.model.state_dict(), name)
    ...
    def predict(self, query_lines: list):
        ...
        test_loader = self.mask_trainer.prepare_test_loader(test_dataset)
        # test_loader = self.mask_trainer.prepare_test_loader(test_dataset, padding_func_type="TypeB")
        # test_loader = self.mask_trainer.prepare_test_loader(test_dataset, padding_func_type="TypeB")
        outputs, labels = list(), list()
        self.mask_trainer.model.eval()
        with torch.no_grad():
            for(x, y) in test_loader:
                x, y = recursive_to_device((x, y), self.args.device)
                output = self.mask_trainer.model(x)
...
def main(args):
    ...
    pred_stat = PredictionStatistics()
    if args.model_type == 'CEB_MSCN':
        mask_trainer = ERMTrainer(args, model.net, optimizer=model.optimizer, model_wrapper=model)
        base = np.e
    else:
        mask_trainer = ERMTrainer(args, model)
        base = 2.0
    ...
    if args.model_save_path is not None and os.path.exists(args.model_save_path): 
        ...
        print("Model save path:", name)
        if os.path.exists(name):
            mask_trainer.model.load_state_dict(torch.load(name))
        else:
            mask_trainer.train(train_dataset)
            torch.save(mask_trainer.model.state_dict(), name)
    else:
        mask_trainer.train(train_dataset)
    if args.model_type == "CEB_MSCN" and args.dump_prediction == 1:
        errors, test_out = mask_trainer.test(test_dataset, return_out=True)
        train_errors, train_out = mask_trainer.test(train_dataset, return_out=True)
        ...
        # debug_errors = mask_trainer.test(debug_dataset)
    else:
        errors = mask_trainer.test(test_dataset)
    # train_errors = mask_trainer.test(train_dataset)
